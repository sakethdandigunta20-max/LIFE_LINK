...
async function generateReport(req, res, next) {
  try {
    ...
    if (!REPORT_QUERIES[type]) {
      return error(res, "Unknown report type", 400);
    }

    // PostgreSQL
    const result = await pool.query(REPORT_QUERIES[type]);
    ...
    if (format === "pdf") {
      ...
      doc.fontSize(18).text(REPORT_TITLES[type], {
        align: "center",
      });
      ...
      if (!rows.length) {
        ...
        const colWidth =
          (doc.page.width - 80) / columns.length;
        ...
        doc
          .rect(40, headerY, doc.page.width - 80, 18)
          .fill("#b91c1c");
        ...
        rows.forEach((row, i) => {
          ...
          if (i % 2 === 0) {
            doc
              .rect(40, rowY, doc.page.width - 80, 16)
              .fill("#f3f4f6")
              .fillColor("black");
          }
